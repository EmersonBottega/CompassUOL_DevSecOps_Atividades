...
2) - Implantar um Deployment chamado "my-deployment" com três réplicas de uma aplicação baseada na imagem "httpd". Atualize a imagem do Deployment para uma versão mais recente. [MyDeployment](#2-deployment-no-terminal-execute-os-comandos-abaixo)
3) - Criar um ConfigMap chamado "app-config" com uma variável de configuração personalizada. - Monte o ConfigMap em um pod e verifique se o valor foi aplicado corretamente. [ConfigMap](#3-configmap-no-terminal-execute-os-comandos-abaixo)
4) - Criar um Secret chamado "app-secret" contendo informações sensíveis. Injete o Secret como uma variável de ambiente em um pod e teste se está acessível. [Secret](#4-secret-no-terminal-execute-os-comandos-abaixo)
5) - Configurar um PersistentVolume de 1Gi de armazenamento local e vincule-o a um PersistentVolumeClaim. Monte o volume em um pod e salve arquivos para verificar a persistência. [PersistentVolume, PersistentVolumeClaim](#5-persistentvolume-e-persistentvolumeclaim)
6) - Criar um serviço do tipo ClusterIP para um Deployment chamado "backend" e teste a conectividade interna entre pods usando o nome do serviço. [ClusterIP](#6-clusterip-no-terminal-execute-os-comandos-abaixo)
7) - Implantar um Job chamado "batch-job" que execute um comando simples e termine. Verifique os logs do Job para confirmar sua execução. [BatchJob](#7-batchjob)
8) - Criar um Horizontal Pod Autoscaler para um Deployment chamado "hpa-deployment" e configure-o para escalar com base no uso de CPU. Aumente a carga e observe o escalonamento. [HorizontalPodAutoscaler](#8-hpa-no-terminal-execute-os-comandos-abaixo)
9) - Criar um serviço do tipo NodePort para expor externamente um Deployment chamado "webapp". Acesse o serviço usando o endereço IP do Minikube e a porta atribuída. [NodePort](#9-nodeport-no-terminal-execute-os-comandos-abaixo)
10) - Criar um pod chamado "restart-pod" com a política de reinício configurada como "OnFailure". - Provoque uma falha no pod e observe seu comportamento. [RestartPod](#10-restartpod)
